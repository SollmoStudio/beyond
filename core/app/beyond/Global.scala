...
object Global extends WithFilters(RequestsCountFilter, TimeoutFilter) with Logging {
  ...
  override def onLoadConfig(defaultConfig: PlayConfiguration, path: File, classLoader: ClassLoader, mode: PlayMode.Mode): PlayConfiguration = {
    def loadConfigFromFile(configName: String): PlayConfiguration = {
      val file = new File(path, s"conf/$configName")
      if (file.exists) {
        logger.info(s"Load ${file.getCanonicalPath}")
        PlayConfiguration(ConfigFactory.parseFile(file))
      } else {
        PlayConfiguration.empty
      }
    }
