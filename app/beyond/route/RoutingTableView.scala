...
class RoutingTableView(currentServer: Address, data: JsArray = new JsArray) extends Logging {
  ...
  def queryServerToHandle(hash: Hash): ServerToHandle = {
    ...
    def whoWillHandle(hash: Hash): Address = {
      // Basically, find the first server that the hash of server is less than or equal to hash of the request.
      //  There are two exception cases.
      //   1. When the server topology is like to above and hash of the request is below 100.
      //    This request have to handled in the first server.
      //   2. The other is when the routing table is empty.
      //    Actually, it's abnormal case. It may be routing table synchronization failure for some reason.
      //    In this case, we handle requests in current server.
      //    It's possible because each server doesn't have internal states, and consistent hashing is just for load balancing.
