...
class BeyondSupervisor extends Actor {
  ...
  override def preStart() {
    ...
    // There is no priority between routing table update message and other messages.
    //  So updating routing table is not immediately applied, but is applied after all requests, that are already in mailbox, are handled.
    //  It's not a problem because beyond server is designed to ensure eventual consistency not strong consistency.
  ...
  override def receive: Receive = {
    case msg: UpdateRoutingTable =>
      ...
      Akka.system.actorSelection(UserActionActorPath) ! (Broadcast(msg), sender)
    case _ =>
  }
}
